...
5. Không tự hủy process/tool session đang chạy bằng `kill`/`pkill`/`killall`.
...
Guard cũng chặn `kill`, `pkill`, `killall`, và pipeline `xargs kill`. `kill -0 <pid>` vẫn được phép vì chỉ là probe kiểm tra process tồn tại. Nếu command/tool session bị treo, model phải báo session/process và hỏi người dùng trước khi dừng.
